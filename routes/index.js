  ...
  db.query(queryStr, function (err, rows, fields) {
    if (!err) {
      ...
      if (Object.keys(rows).length > 0 && rows[0].fcm_token) {
        sendFcm(rows[0].fcm_token, '배치가 완료되었습니다.');
      } else {
  ...
  db.query(queryStr, function (err, rows, fields) {
    if (!err) {
      ...
      if (err.code == 'ER_DUP_ENTRY') {
        ...
        res.json([
          { code: 3, message: '알 수 없는 오류가 발생하였습니다.', data: err },
        ]);
      }
    }
  });
...
router.post('/taxi/list', function (req, res) {
  ...
  db.query(queryStr, function (err, rows, fields) {
    if (!err) {
      ...
      console.log('err : ' + err);
      res.json([
        { code: 1, message: '알 수 없는 오류가 발생하였습니다.', data: err },
      ]);
    }
  });
});
...
router.post('/driver/list', function (req, res) {
  console.log('dirver-list / req.body ' + JSON.stringify(req.body));

  let userId = req.body.userId;

  console.log('dirver-list / userId = ' + userId);
  ...
  db.query(queryStr, function (err, rows, fields) {
    if (!err) {
      ...
      if (rows.affectedRows > 0) {
        ...
        res.json([
          { code: 2, message: '이미 완료되었거나 존재하지 않는 call 입니다.' },
        ]);
      }
    } else {
